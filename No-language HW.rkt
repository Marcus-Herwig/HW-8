...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname |No-language HW|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(define resolve
  (lambda (varName env)
    (cond
      ((null? env) #f)
...
(define let-extend-env
  (lambda (var val env)
    (cond
      ((null? var) env)
      (else (list (resolve(run-parsed-code var env)env) (resolve(run-parsed-code val env)env) env)))))                          
...
(define no-parser
  (lambda (no-code)
    (cond
      ...
      ((eq? (car no-code) 'let)
        (list 'let-exp (no-parser(car(car(car(cdr no-code))))) (no-parser(cadr(car(car(cdr no-code))))) (no-parser(car(cadr(car(cdr no-code))))) (no-parser(cadr(cadr(car(cdr no-code)))))(no-parser(car(cdr(cdr no-code))))))
  ...
  (lambda (parsed-no-code env)
    (cond
     ...
     ((eq? (car parsed-no-code) 'let-exp)
      (do-mathy-stuff-toaster (cadr(car(reverse parsed-no-code)))
                                              (resolve(cadr(caddr(car(reverse parsed-no-code)))) (extend-env (list(cadr(cadr parsed-no-code))(cadr(cadddr parsed-no-code))) (list(car(cdr(caddr parsed-no-code))) (cadr(cadr(reverse parsed-no-code)))) env))
                                              (resolve(cadr(cadddr(car(reverse parsed-no-code)))) (extend-env (list(cadr(cadr parsed-no-code))(cadr(cadddr parsed-no-code))) (list(car(cdr(caddr parsed-no-code))) (cadr(cadr(reverse parsed-no-code)))) env))))
...
(define sample-no-code '(ask (< 2 5) (let ((f 4) (h 3)) (do-mathy-stuff + f h)) otherwise c))
